  ...
  while (1) {
    ...
    if (word_count - last_word_count > 10000) {
      ...
      if ((debug_mode > 1)) {
        now = clock();
        printf(
            "%cAlpha: %f  Progress: %.2f%%  Words/thread/sec: %.2fk  Loss: %.0f",
            13, alpha,
            word_count_actual / (real)(iter * vocabulary.train_words + 1) * 100,
            word_count_actual /
                ((real)(now - start + 1) / (real)CLOCKS_PER_SEC * 1000),
            (loss > 10000000 ? 10000000 : loss));
        fflush(stdout);
        loss = 0;
      }
      alpha =
    ...
    if (feof(fi) || (word_count > vocabulary.train_words / num_threads)) {
      word_count_actual += word_count - last_word_count;
      local_iter--;
      if (local_iter == 0)
        break;
          ...
          if (negative > 0) {
            for (d = 0; d < negative + 1; d++) {
              ...
              if (f > MAX_EXP) {
                ...
                g = (label - 1) * alpha;
                if (d == 0) {
                  // loss -= log(exp(f)) for positive
                  // loss -= 0; // do nothing
                } else {
                  // loss -= log(exp(-f)) for negative
                  loss = 10000000;
                }
              } else if (f < -MAX_EXP) {
                g = (label - 0) * alpha;
                ...
                g = (label - expTable[(int)((f + MAX_EXP) *
                    ...
                    alpha;
                if (d == 0) {
                  loss -= log(expTable[(int)((f + MAX_EXP) *
                                             (EXP_TABLE_SIZE / MAX_EXP / 2))]);
                } else {
                  loss -= log(expTable[(int)((-f + MAX_EXP) *
                                             (EXP_TABLE_SIZE / MAX_EXP / 2))]);
...
void TrainModel() {
  ...
  if (read_vocab_file[0] != 0) {
    ...
    // get training file_size
    printf("getting training file size\n");
    ...
    file_size = LearnVocabFromTrainFile(vocabulary, train_file);
  ...
  if (classes == 0) {
    ...
    printf("Word vector saved at '%s'\n", output_file);
    // Save syn1
    if (hs) {
      strcpy(output_file1, output_file);
      strcat(output_file1, "_1");
    ...
    int *cl = (int *)calloc(vocabulary.vocab_size, sizeof(int));
    real closev, x;
    ...
    for (a = 0; a < iter; a++) {
      ...
      for (b = 0; b < clcn; b++)
        centcn[b] = 1;
      for (c = 0; c < vocabulary.vocab_size; c++) {
