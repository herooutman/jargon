...
int main(int argc, char **argv) {
  ...
  char *vocab;
  int i;
  int TCN, CCN = 0, TACN = 0, CACN = 0, SECN = 0, SYCN = 0, SEAC = 0, SYAC = 0,
           QID = 0, TQ = 0, TQS = 0;
  if (argc == 1) {
    printf("Usage: ./compute-accuracy -model <FILE> -th <threshold> -binary "
           "<BINARY>\nwhere FILE contains "
           "word projections, and threshold is used to reduce vocabulary of "
           "the model for fast approximate evaluation (0 = off, otherwise "
           "typical value is 30000)\n");
    return 0;
  }
  ...
  if ((i = ArgPos((char *)"-th", argc, argv)) > 0)
  ...
  fclose(f);
  printf("finished loading vector file.\n");
  TCN = 0;
  while (1) {
    for (a = 0; a < N; a++)
      bestd[a] = 0;
    ...
    if ((!strcmp(st1, ":")) || (!strcmp(st1, "EXIT")) || feof(stdin)) {
      ...
      if (QID != 0) {
        printf("ACCURACY TOP1: %.2f %%  (%d / %d)\n", CCN / (float)TCN * 100,
               CCN, TCN);
        printf("Total accuracy: %.2f %%   Semantic accuracy: %.2f %%   "
